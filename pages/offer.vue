    ...
    >
      <div class="container">
        ...
        <div v-if="offersCar.length > 0" class="includedOffers">
          <h3 class="fw-bolder pt-3 thehead">
            {{ $t("Vehicles Included In The Offer") }}
          </h3>
          <div
            class="row justify-content-center justify-content-xl-start justify-content-lg-start gap-1"
          >
            <div
              v-for="car in offersCar"
              style="cursor: pointer"
              class="col-xl-2 col-lg-2 col-md-4"
            >
              <div
                class="new-car-card"
...
const offerFanc = async () => {
  ...
  if (process.client) {
    ...
    for (let i = 0; i < box.data.offer_cars.length; i++) {
      box.data.offer_cars[i].is_in_favorite =
        favGet.find((element) => element.id == box.data.offer_cars[i].id) !=
        undefined;
    }
  }
  ...
  useHead({
    title:
      lang.value == "ar"
        ? `${theOffer.value.title}/الكثيري للسيارات`
        : `alkathiri motors / ${theOffer.value.title}`,
  });
};
offerFanc();
