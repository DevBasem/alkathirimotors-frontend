        ...
        <div class="container bookPart d-flex flex-column gap-3 align-items-center justify-content-center" style="margin-top: 150px; min-height: 100vh;">
         ...
         <p>visit id:   <span v-if="id">{{ id }}</span></p>
          ...
          <p>{{ $t('time') }}:   <span v-if="time">{{ time }}</span></p>            
           ...
           <div class="col">
          <p>{{ $t('city') }}:   <span v-if="city">{{ city }}</span></p>
          <p>{{ $t('branch') }}:   <span v-if="branch">{{branch}}</span></p> 
           </div>
          </div>
          <div class="btns d-flex align-items-center gap-3">
          <nuxt-link :to="localePath('/')">
                     <button class="custom-btn">{{ $t('back to home') }}</button>
          </nuxt-link>
          <nuxt-link :to="localePath('/maintenance')">
            <button class="back custom-btn stroke" style="background-color: #fff !important; color:#1b395f !important;">{{ $t('back to another appointment') }}</button>
          </nuxt-link>
          </div>
        </div>
    </div>
