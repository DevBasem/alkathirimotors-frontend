              ...
              <div class="accordion" id="accordionExample">
                <div class="accordion-item mainItem">
                  <h2 class="accordion-header">
                    <button class="accordion-button   mainbtnacc" type="button" data-bs-toggle="collapse"
                      ...
                      <div class="checksCars">
                        <div class="inp" v-for="box, index in filterData.brands">
                          <input :id="`label${index}`" type="checkbox" :checked="mainId == box.id" v-model="checkMain"
                            :value="box.id">
                          <label @click="mainName = '' " :for="`label${index}`" class="d-flex align-items-center gap-2">
                ...
                <div class="accordion-item mainItem">
                  ...
                  <div v-if="checkMain.length >= 1" id="collapseOne" class="accordion-collapse collapse show"
                            ...
                            <div v-if="boxi.id == item" class="accordion-header w-100">
                              <button v-for="model in boxi.parent_models" class="accordion-button  d-flex align-items-center gap-3" type="button"
                                ...
                                <div  class="d-flex align-items-center gap-2">
                                  ...
                                  <label :for="`box-${model.id}`">{{ model.name }}</label>
                                </div>
      ...
      <div class="accordion-body ">
         <div class="checks d-flex flex-column gap-3">
            ...
            <span>{{ box.name }}</span>
              ...
              <button @click="showCars()" :disabled="isLoading" class="my-3 custom-btn w-100">{{ $t('apply') }}</button>
            </div>
          </div>
...
let mainName = ref(route.query.name ? route.query.name : null);
...
const showCars = async () => {
  ...
  let result = await axios.get(`${url}/cars`, {
    params: {
      brands: checkMain.value.length > 0 ? checkMain.value : [],
      min_price: minNum.value,
      max_price: maxNum.value,
      models: mainName.value ? models.value = [] : models.value,
      name: mainName.value,
...
onMounted(() => {
  ...
  showCars();
  useHead({
    title: currentLang.value == 'ar' ? `${mainName.value || 'السيارات'}/الكثيري للسيارات` : `alkathiri motors / ${mainName.value || 'cars'}`
  });
});
