<template>
  <div style="min-height: 100vh; margin-bottom: 50px;">
    <div class="carName" style="margin-top: 80px">
      <div v-for="car in mainCar.cars">
        <div v-if="carDetails == car.id">
          ...
          <div
            class="d-flex align-items-center justify-content-end gap-3 mx-4 my-3"
          >
            <img
              style="cursor: pointer"
              data-bs-toggle="modal"
              data-bs-target="#shareViaModal"
          ...
          <div v-if="car.price_field_status == 'show' " class="row gap-3 justify-content-between align-items-center">
            <div class="col-3 prices">
              <div class="price d-flex align-items-center gap-5 w-100">
                <div
                
                  class="thePrice d-flex flex-column gap-1 text-center py-2 px-4"
                >
                  <span>{{ $t("price") }}</span>
                  <span class="fw-bold" style="font-size: 17px"
                    >{{ car.selling_price }} {{ $t("Sar") }}</span
                  >
                </div>
                <div  class="after text-center gap-1 d-flex flex-column">
                  <span>{{ $t("after vat") }}</span>
                  <span class="fw-bold" style="font-size: 17px"
                    >{{ car.selling_price_after_vat }} {{ $t("Sar") }}</span
                  >
                </div>
              </div>
            </div>
            <div class="col-12 col-xl-3 col-lg-3 maindivColor">
              <div
                class="colors d-flex align-items-center justify-content-between"
              >
                ...
                <div class="theColors">
                  <span
                    ...
                    @click="colorCar = color.id"
                    :class="{ active: colorCar == color.id }"
                  ></span>
                </div>
                <div v-for="(color, index) in car.colors">
                  <div
                    v-if="colorCar == color.id"
                    class="mainColor"
                    :style="{ 'background-color': color.hex_code }"
                  >
                    <p
                      ...
                      :class="{
                        white: color.name == 'ابيض' || color.name == 'white',
                      }"
                    >
                      {{ color.name }}
                    </p>
            ...
            <div
              class="col-12 col-xl-3 col-lg-3 px-4 d-flex justify-content-end mainBtn"
            >
              <button
                @click="goToPurchasePage(car.id, colorCar)"
                class="purchaseBtn custom-btn"
                style="padding:15px 75px !important;"
              >
                {{ $t("order") }}
              </button>
            </div>
          </div>
          <div v-if="car.price_field_status == 'available_on_request' " class="row gap-3 justify-content-between align-items-center">
            <div class="col-3 prices">
              <div class="price d-flex align-items-center gap-5 w-100">
                <div
                
                  class="thePrice d-flex flex-column gap-1 text-center px-4" style="padding: 20px 0px;"
                >
                  <span classs="" style="font-weight: 700;">{{ $t("request1") }}</span>
          ...
          <div v-if="car.price_field_status == 'unavailable' " class="row gap-3 justify-content-between align-items-center">
            <div class="col-3 prices">
              <div class="price d-flex align-items-center gap-5 w-100">
                <div
                  ...
                  class="thePrice d-flex flex-column gap-1 text-center px-4" style="padding: 20px 0px;"
                >
                  <span classs="" style="font-weight: 700;">{{ $t("request2") }}</span>
          ...
          <div v-if="car.price_field_status == 'competitive_price' " class="row gap-3 justify-content-between align-items-center">
            <div class="col-3 prices">
              <div class="price d-flex align-items-center gap-5 w-100">
                <div
                  ...
                  class="thePrice d-flex flex-column gap-1 text-center px-4" style="padding: 20px 0px;"
                >
                  <span classs="" style="font-weight: 700;">{{ $t("request3") }}</span>
          ...
          <div v-if="car.price_field_status == 'other' " class="row gap-3 justify-content-between align-items-center">
            <div class="col-3 prices">
              <div class="price d-flex align-items-center gap-5 w-100">
                <div
                  ...
                  class="thePrice d-flex flex-column gap-1 text-center px-4" style="padding: 20px 0px;"
                >
                  <span classs="" v-if="car.field_other_status" style="font-weight: 700;">{{ car.field_other_status }}</span>
    ...
    <div
      ...
      tabindex="-1"
      role="dialog"
      aria-labelledby="shareViaModalLabel"
      aria-hidden="true"
    >
          ...
          <div class="modal-footer w-100">
            <div class="inp w-100">
              <input
                ...
                class="col-11 ur"
                type="url"
                :placeholder="route ? route : 'loading..'"
                id="myInput"
  ...
  '1024': {
    ...
    <div class="container mainCarDetails">
      <div class="row justify-content-center gap">
        <div class="col-12 col-xl-7 col-lg-7 col-md-8">
          <div class="settings my-5">
            <div
              v-for="car in mainCar.cars"
              class="container settings-container"
            >
              <div
                v-if="carDetails == car.id"
                class="row text-cente align-items-center justify-content-center justify-content-xl-start justify-content-lg-start p-2"
              ...
              >
                ...
                <div class="col-5 col-xl-3 col-lg-3 col-md-3 my-2">
                  <div class="setting-box">
                    <img
                      src="@/assets/images/noun-transmission-1325127.svg"
                      alt=""
                    />
          ...
          <div class="Alldescription containe">
            ...
            <div class="container">
              <swiper
                ...
                :navigation="true"
                :modules="[SwiperAutoplay, SwiperPagination, SwiperNavigation]"
                class="mySwiper swiper-car-category"
                style="
                  position: sticky;
                  top: 80px;
                  z-index: 555;
              ...
              >
                <swiper-slide
                  v-for="(i, index) in mainCar.cars"
                  @click="carDetails = i.id"
        ...
        <div class="col-12 col-xl-5 col-lg-5 mainCarNameContainer">
          <div
            v-for="car in mainCar.cars"
            class="sticky-carDetails d-flex flex-column gap-2"
          >
            <div v-if="carDetails == car.id" class="carNameSlide">
              <div
                class="d-flex align-items-center justify-content-end gap-3 mx-4 mb-2"
              ...
              <select name="" id="" v-model="carDetails">
                <option v-for="car in mainCar.cars" :value="car.id">
                  {{ car.name }}
                </option>
              </select>
              <div
                class="colors d-flex align-items-cente flex-column justify-content-between mx-4"
              >
                <p class="available">{{ $t("Colors Available") }}</p>
              ...
              <span class="text-danger fw-bold text-center px-2 mt-1 mb-2"
                >{{ $t("warning") }}
              </span>
              <div v-if="car.price_field_status == 'unavailable' " class="price">
              <div  class="thePrice text-center d-flex flex-column gap-1 px-4 w-50" style="padding: 17px 0px;">
              <span style="font-weight: 700;"> {{ $t('request2') }}</span>
              </div>
              
              </div>
              <div v-if="car.price_field_status == 'other' " class="price">
              <div  class="thePrice text-center d-flex flex-column gap-1 px-4 w-50" style="padding: 17px 0px;">
              <span v-if="car.field_other_status" style="font-weight: 700;"> {{ car.field_other_status }}</span>
              </div>
              
              </div>
              <div v-if="car.price_field_status == 'competitive_price' " class="price">
              <div  class="thePrice text-center d-flex flex-column gap-1 px-4 w-50" style="padding: 17px 0px;">
              <span style="font-weight: 700;"> {{ $t('request3') }}</span>
              </div>
              
              </div>
              <div v-if="car.price_field_status == 'available_on_request' " class="price">
              <div  class="thePrice text-center d-flex flex-column gap-1 px-4 w-50" style="padding: 17px 0px;">
              <span style="font-weight: 700;"> {{ $t('request1') }}</span>
              </div>
              
              </div>
              <div
              v-if="car.price_field_status == 'show'"
                class="price d-flex align-items-center gap-5 w-100"
                style="white-space: normal"
              >
                <div
                  class="thePrice text-center d-flex flex-column gap-1 py-2 px-4 w-50"
                >
                  <span>{{ $t("price") }}</span>
                  <span class="fw-bold" style="font-size: 17px"
                    >{{ car.selling_price }} {{ $t("Sar") }}</span
...
export default {
  ...
  async setup() {
    ...
    const carLatestFunc = async () => {
      ...
      carDetails.value = car.data.cars[0].id;
    ...
    const addToFav = async (car) => {
      ...
      if (process.client) {
        let favGet = JSON.parse(sessionStorage.getItem("thefav"))
          ...
          : [];
        let favChoose = favGet.find((boxi) => boxi.id == box.id);
